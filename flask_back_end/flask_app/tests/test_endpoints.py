...
def test_registration(app) -> None:
    ...
    no_body = client().post("/api/user/registration")
    ...
    no_pw = client().post("/api/user/registration", data={"username": "bob"})
    ...
    no_username = client().post("/api/user/registration", data={"password": "bob"})
...
def test_login(app) -> None:
    ...
    client = app.test_client
    data = {}
    no_data = client().post("/api/user/login", data=data)
    ...
    bad_pw = client().post("/api/user/login", data=data)
...
def test_logout_access(app) -> None:
    ...
    """
    client = app.test_client
    no_token = client().post("/api/logout/access")
    ...
    invalidate_access = client().post("/api/logout/access", headers=headers)
    ...
    revoked_token = client().post("/api/logout/access", headers=headers)
...
def test_logout_refresh(app) -> None:
    ...
    invalidate_refresh = client().post("/api/logout/refresh", headers=headers)
    ...
    revoked_token = client().post("/api/logout/refresh", headers=headers)
    assert 401 == revoked_token.status_code
...
def test_refresh_access(app) -> None:
    ...
    access_token = body["access_token"]
    refresh_token = body["refresh_token"]
    no_token = client().post("/api/token/refresh")
    assert 401 == no_token.status_code
    headers = {"Authorization": f"Bearer {access_token}"}
    bad_token = client().post("/api/token/refresh", headers=headers)
    assert 422 == bad_token.status_code
    headers = {"Authorization": f"Bearer {refresh_token}"}
    good_token = client().post("/api/token/refresh", headers=headers)
...
def test_reset_password(app) -> None:
    ...
    resp = client().post("/api/user/reset_password", query_string={})
    body = json.loads(resp.data.decode("utf-8"))
    assert 400 == resp.status_code
    assert body["message"] == {"username": "This field cannot be blank"}
    resp = client().post("/api/user/reset_password", data={"username": "not@bob.com"})
    assert resp.status_code == 200
    body = json.loads(resp.data.decode("utf-8"))
    assert body["message"] == "Attempted password reset"
    resp = client().post("/api/user/reset_password", data={"username": "bob@bob.com"})
    assert resp.status_code == 200
    assert body["message"] == "Attempted password reset"
