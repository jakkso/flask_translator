...
class UserRegistration(Endpoint):
    def post(self):
        ...
        if UserModel.find_by_username(data["username"]):
            self.logger.info(fmt('POST', data['username'], 'Registration attempt for existing user'))
        ...
        if not new_user.validate_username():
            self.logger.info(fmt('POST', new_user.username, 'Registration attempt with invalid username'))
            return {"message": "Invalid email address"}, 400
        elif not UserModel.validate_password(data["password"]):
            self.logger.info(fmt('POST', new_user.username, 'Registration attempt with invalid password'))
            ...
            try:
                new_user.save_to_db()
                self.logger.info(fmt('POST', new_user.username, 'Successful registration'))
...
class UserActivation(Endpoint):
    def post(self):
        ...
        if not current_user or not UserModel.verify_hash(
            ...
            if not current_user:
                self.logger.info(fmt('POST', 'Bad/missing', 'Failed activation email req: bad/missing username'))
            else:
                self.logger.info(fmt('POST', current_user.username, 'Failed activation email req: bad password'))
            return {"message": "Bad credentials"}, 400
        elif current_user.email_verified:
            self.logger.info(fmt('POST', current_user.username, 'Failed activation email req: already verified'))
            ...
            subject = "Please confirm your email address"
            html = render_template("activate.html", confirm_url=confirm_url)
            self.logger.info(fmt('POST', current_user.username, 'Successful activation email req'))
    ...
    def put(self):
        ...
        if not user or not username:
            ...
            self.logger.info(fmt('PUT', username, 'Failed activation: already activated'))
...
class UserLogin(Endpoint):
    def post(self):
        ...
        if not user or not UserModel.verify_hash(data["password"], user.password):
            if not user:
                self.logger.info(fmt('POST', data['username'], 'Failed login: bad username'))
            else:
                self.logger.info(fmt('POST', data['username'], 'Failed login: bad password'))
            return {"message": "Bad credentials"}, 400
...
class UserLogoutAccess(Endpoint):
    ...
    def post(self):
        ...
        try:
            ...
            revoked_token.add()
            self.logger.info(fmt('POST', username, 'Access token revocation successful'))
            return {"message": "Access token has been revoked"}, 202
        except ExpiredSignatureError:
            self.logger.info(fmt('POST', username, 'Access token revocation failed: expired'))
...
class UserLogoutRefresh(Endpoint):
    ...
    def post(self):
        ...
        try:
            revoked_token = RevokedTokenModel(jti=jti)
            revoked_token.add()
            self.logger.info(fmt('POST', username, 'Refresh token revocation successful'))
            return {"message": "Refresh token has been revoked"}, 202
        except ExpiredSignatureError:
            self.logger.info(fmt('POST', username, 'Refresh token revocation failed: expired'))
            return {"message": "Something went wrong"}, 500
...
class UserResetPassword(Endpoint):
    def post(self):
        ...
        user = UserModel.find_by_username(data["username"])
        if user:
            self.logger.info(fmt('POST', user.username, 'Successful password reset email req'))
            ...
            self.logger.info(fmt('POST', data['username'], 'Failed password reset email req: bad/missing username'))
    ...
    def put(self):
        ...
        user = UserModel.find_by_username(username)
        if not user or not username:
            self.logger.info(fmt('PUT', username, 'Failed password reset: bad/missing username'))
            return {"message": "Invalid username"}, 400
        elif not user.validate_password(password):
            self.logger.info(fmt('PUT', username, 'Failed password reset: bad/missing password'))
            return {"message": "Invalid password"}, 400
        else:
            user.password = UserModel.generate_hash(password)
            user.save_to_db()
            self.logger.info(fmt('PUT', username, 'Successful password change and access token revocation'))
...
class UserDeletion(Endpoint):
    ...
    def delete(self):
        ...
        if not user or not UserModel.verify_hash(password, user.password):
            if not user:
                self.logger.info(fmt('DELETE', username, 'Failed user deletion: bad/missing username'))
            else:
                self.logger.info(fmt('DELETE', username, 'Failed user deletion: bad/missing password'))
            return {"message": "Bad credentials"}, 400
        revoked_token = RevokedTokenModel(jti=token["jti"])
        revoked_token.add()
        self.logger.info(fmt('DELETE', username, 'Successful user deletion and access token revocation'))
...
class SecretResource(Endpoint):
    ...
    def post(self):
        ...
        if isinstance(response, list):  # Successful responses are lists of dicts
            self.logger.info(fmt('POST', user, 'Successful text translate request made'))
            return response, 200
        elif response["error"]:
            self.logger.error(fmt('POST', user, f'Failed text translate request: {response["error"]}'))
